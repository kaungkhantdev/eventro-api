...
describe('Roles Decorator', () => {
  ...
  it('should set multiple roles metadata', () => {
    @Roles(UserRole.ADMIN, UserRole.ATTENDEE)
    class TestClass {}

    const metadata = Reflect.getMetadata(ROLES_KEY, TestClass);
    expect(metadata).toEqual([UserRole.ADMIN, UserRole.ATTENDEE]);
  });
  ...
  it('should work on methods', () => {
    class TestController {
      ...
      @Roles(UserRole.ATTENDEE, UserRole.ORGANIZER)
    ...
    expect(multiRoleMetadata).toEqual([UserRole.ATTENDEE, UserRole.ORGANIZER]);
